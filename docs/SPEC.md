# 灵枢 Agent 产品与技术规格（SPEC）

> 版本：v0.1 · 状态：规格起草 · 平台：Windows 10/11

## 1. 项目概述

| 项 | 说明 |
|----|------|
| **名称** | 灵枢 Agent（SenseHub Agent） |
| **定位** | Windows 本地多模态感知 + 任务规划 + 桌面自动化的闭环人机交互 Agent |
| **部署形态** | 本地常驻服务（Python 后端）+ 本机/局域网 Web 控制台；不默认暴露公网 |
| **核心闭环** | 感知（视/听/屏/指令）→ 理解（LLM + 规则）→ 规划（LangGraph）→ 执行（桌面/网页）→ 反馈（截图/状态/语音） |
...
技术栈：MediaPipe Hands + OpenCV 透视变换 + 防抖算法。
...
详见 [`TIERS.md`](TIERS.md)。摘要：

| 维度 | Lite | Pro | Max |
|------|------|-----|-----|
| 文本指令 | 每日 20 次 | 无限 | 无限 |
| 语音 | 离线小模型 + 10 快捷命令 | 流式 ASR | 最低延迟 + 唤醒词 |
| 视觉 | 预览 + 人员检测 | + 手势规则 | + 虚拟屏 + VLM |
| LLM | 自填 Key 或限额 | 增强额度 | 最强 + 多模型协作 |
| 规则 | 最多 3 条 | 50 条 | 无限 + 组合条件 |
| 远程 | 仅 localhost | 局域网 | 局域网 + 安全隧道 |

授权：本地 License + 可选在线校验；功能门控在中间件层统一拦截。
...
| 阶段 | 内容 |
|------|------|
| **Phase 0** | 规格文档、脚手架、环境验收 |
| **Phase 1 MVP** | 文本指令 + LangGraph + 桌面/网页执行 + Web UI + 基础安全 |
| **Phase 2** | 摄像头 + YOLO + Whisper + 规则引擎 + 多模型路由 |
| **Phase 3 Pro** | 流式 ASR、手势、TTS、局域网、档位授权 |
| **Phase 4 Max** | 虚拟屏、VLM 验证、多 Agent 协作、安全远程隧道 |
