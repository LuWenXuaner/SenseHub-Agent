...
> **文档版本**：v1.0（商业化交付版）  
> **目标**：一套可扩展的企业级控制台界面，覆盖 Console / Studio / Code 全产品线。  
> **主题**：浅色 + 深色双主题（支持跟随系统）。  
> **档位策略**：四档订阅（Lite / Standard / Pro / Max），三档运行时门控；同一套 UI，TierGate 引导升级。
...
| **档位可感知** | Lite / Standard / Pro / Max 共用同一套 UI；不可用能力 **可见但锁定**，引导 Token Plan 升级 |
...
## 5. 档位 UI 策略（Lite / Standard / Pro / Max）

**同一应用、同一导航**；运行时按 `lite` / `pro` / `max` 门控，Standard 与 Pro 界面表现一致，差异在 Token Plan 页展示配额与 API 权益。
...
|----------|----------|---------|
| 有权限 | 可用 | 正常交互 |
| 无权限（低档位） | 锁定 | 卡片可见 + 锁图标 +「Standard / Max 可用」+ 升级 CTA |
| 超额（如 Lite 20 次/日） | 限流 | Toast + 用量条变红 + 跳转 Token Plan |

### 5.2 导航与页面可见性

| 导航项 | Lite | Standard / Pro | Max |
|--------|------|----------------|-----|
| Console / Studio / Code 入口 | ✓ | ✓ | ✓ |
| 摄像头预览 | ✓ | ✓ | ✓ |
| 手势 / 场景规则 | 锁定 | ✓ | ✓ |
| 虚拟屏校准 | 锁定 | 锁定 | ✓ |
| 规则数量 | 3 条上限提示 | 50 条 | 不限 |
| 安全中心 / 局域网 | 简版 | 完整 | 完整 |
| 审计导出 | 锁定 | 90 天 | 1 年 + 导出 |
...
- **Lite**：灰色标识
- **Standard**：蓝色标识（生效 pro）
- **Pro**：靛蓝标识（生效 pro，附 Token / API 权益提示）
- **Max**：渐变描边标识

点击跳转 Token Plan 或控制台「积分兑换」。
